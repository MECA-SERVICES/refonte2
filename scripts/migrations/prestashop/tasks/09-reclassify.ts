...
export const reclassifyTask: Task = {
	...
	async run({ dryRun, limit }) {
		...
		for (;;) {
			...
			for (const p of page) {
				...
				const path = hit
					...
					: `${PIECES_ROOT} > ${UNCLASSIFIED}`;

				const categoryId = hit ? idByPath.get(path) : unclassifiedId;
		...
		return {
			processed: updated,
			note: `${count(matched)} par règle, ${count(unmatched)} à classer, ${count(deactivated)} désactivés`
		};
	}
};
